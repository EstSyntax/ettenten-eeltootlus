...
class Processor(object):
    ...
    def process_file(self, file_path, output_dir_path):
        ...
        with open(file_path) as fid, open(output_path, 'w') as fod:
            ...
            # print(loigu_eraldamine)
            self.process(loigu_eraldamine, fod)
    ...
    def process_directory(input_dir, output_dir, count_sentences):
        ...
        xmls = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if
                f.endswith('.url')]  # enne oli siin if f.endswith('.xml')
